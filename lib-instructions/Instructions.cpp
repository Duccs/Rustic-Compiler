...
void InstructionsClass::WriteEndlLinux64()
{
    ...
    Encode(BIT64);
    Encode(IMMEDIATE_TO_ESI);
    Encode(&mNewLineString); // address of bytes to print
...
int *InstructionsClass::GetMem(int index)
{
    ...
    {
        std::cerr << "Error. Data index " << index << " is out of bounds. Max: "
                  << MAX_DATA - 1 << std::endl;
        exit(1);
    }
}
...
// Overloaded Encode to put a 4-byte integer into mCode:
...
// Overloaded Encode to put an 8-byte long long into mCode:
...
// Overloaded Encode to put an 8 or 4-byte pointer into mCode:
...
void InstructionsClass::PushVariable(int variableIndex)
{
    Encode(MEM_TO_EAX);
    Encode(GetMem(variableIndex));
    Encode(PUSH_EAX);
}

void InstructionsClass::PopAndStore(int variableIndex)
{
    Encode(POP_EAX);
    Encode(EAX_TO_MEM);
    Encode(GetMem(variableIndex));
...
void InstructionsClass::PopPopAddPush()
{
    Encode(POP_EBX);
    Encode(POP_EAX);
    Encode(ADD_EAX_EBX1); // ADD EAX to EBX. Result in EAX.
    Encode(ADD_EAX_EBX2);
    Encode(PUSH_EAX);
}
...
void InstructionsClass::PopPopSubPush()
{
    Encode(POP_EBX);
    Encode(POP_EAX);
    Encode(SUB_EAX_EBX1); // Subtract EAX from EBX. Result in EAX.
    Encode(SUB_EAX_EBX2);
    Encode(PUSH_EAX);
}
